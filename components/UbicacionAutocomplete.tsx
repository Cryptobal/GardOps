...
interface UbicacionAutocompleteProps {
  value?: string
  onSelect: (ubicacion: UbicacionData) => void
  placeholder?: string
  label?: string
  required?: boolean
  className?: string
...
export function UbicacionAutocomplete({
  value = '',
  onSelect,
  ...
  // Función para extraer datos de ubicación desde Google Places
  const extractLocationData = (place: google.maps.places.PlaceResult): UbicacionData => {
    ...
    // Extraer comuna y ciudad de address_components
    if (place.address_components) {
      for (const component of place.address_components) {
        ...
        } else if (!ciudad && types.includes('administrative_area_level_2')) {
          ciudad = component.long_name
        }
      }
    }
    ...
    placesService.current.getDetails(
      ...
      (place, status) => {
        if (status === google.maps.places.PlacesServiceStatus.OK && place) {
          const ubicacionData = extractLocationData(place)
          setInputValue(ubicacionData.direccion)
          onSelect(ubicacionData)
        } else {
