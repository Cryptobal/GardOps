-- =====================================================
-- FIX CORREGIDO - IDs Reales en Vista Central
-- =====================================================

-- Este script corrige el error de tipos en la vista
-- El problema era que cl.id es bigint y gen_random_uuid() es uuid

-- 1. Crear la vista corregida directamente
...
unidos_con_estado AS (
  SELECT 
    -- CORRECCIÓN CLAVE: Convertir cl.id a uuid para compatibilidad
    COALESCE(
      cl.id::uuid, 
      gen_random_uuid()
    ) AS id,
...
LEFT JOIN central_llamados c ON c.id::uuid = v.id
