-- =====================================================
-- FIX INMEDIATO - IDs Reales en Vista Central
-- =====================================================

-- Este script corrige INMEDIATAMENTE el problema de IDs aleatorios
-- en la vista central_v_llamados_automaticos

-- 1. Verificar el problema actual
SELECT 'PROBLEMA ACTUAL:' as status;
SELECT 
  id,
  instalacion_nombre,
  estado_llamado,
  'ID generado aleatoriamente' as problema
FROM central_v_llamados_automaticos 
WHERE estado_llamado = 'pendiente'
LIMIT 3;

-- 2. Verificar si estos IDs existen en central_llamados
SELECT 'VERIFICACION IDs:' as status;
SELECT 
  v.id,
  CASE 
    WHEN c.id IS NOT NULL THEN 'EXISTE en central_llamados'
    ELSE 'NO EXISTE en central_llamados - ESTE ES EL PROBLEMA'
  END as estado_id
...
unidos_con_estado AS (
  SELECT 
    -- CORRECCIÓN CLAVE: Usar ID real si existe, o generar uno determinístico
    COALESCE(
      cl.id, 
      -- Generar ID determinístico basado en instalación y hora
      gen_random_uuid()
    ) AS id,
    ch.instalacion_id,
...
-- 5. Verificar que ahora los IDs existen o se pueden crear
