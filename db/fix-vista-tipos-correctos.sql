-- =====================================================
-- FIX FINAL - Tipos de Datos Correctos
-- =====================================================

-- El problema era que pauta_id es bigint, no uuid
-- Necesitamos usar los tipos correctos para cada campo

-- 1. Crear la vista con tipos correctos
...
unidos_con_estado AS (
  SELECT 
    -- CORRECCIÓN: Usar tipos correctos para cada campo
    COALESCE(cl.id, gen_random_uuid()) AS id,
    ch.instalacion_id,
    COALESCE(cl.guardia_id, NULL::uuid) AS guardia_id,
    COALESCE(cl.pauta_id, NULL::bigint) AS pauta_id,  -- CORREGIDO: bigint, no uuid
...
SELECT 
  ...
  instalacion_nombre,
  estado_llamado,
  'Vista corregida - Tipos correctos' as solucion
