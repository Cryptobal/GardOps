...
async function investigarProblemaPPC() {
  ...
  try {
    ...
    const aaronResult = await pool.query(`
      SELECT 
        g.id,
        g.nombre,
        g.apellido_paterno,
    ...
    aaronResult.rows.forEach(row => {
      console.log(`  - ID: ${row.id}, Nombre: ${row.nombre} ${row.apellido_paterno} ${row.apellido_materno}`);
    });
    ...
    console.log('\n5️⃣ Buscando específicamente en Bodega Santa Amalia...');
    const santaAmaliaResult = await pool.query(`
      SELECT 
        pauta_id,
        puesto_id,
        instalacion_nombre,
    ...
    console.log('📋 Registros en Santa Amalia para hoy:', santaAmaliaResult.rows.length);
    santaAmaliaResult.rows.forEach(row => {
      console.log(`  - Puesto: ${row.puesto_nombre}, Estado: ${row.estado_ui}, PPC: ${row.es_ppc}, Trabajo: ${row.guardia_trabajo_nombre || 'Sin asignar'}`);
      if (row.meta) {
