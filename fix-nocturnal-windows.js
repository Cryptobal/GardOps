...
async function fixNocturnalWindows() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 CORRIGIENDO VENTANAS NOCTURNAS\n');
    ...
    console.log('2️⃣ Creando vista con lógica nocturna...');
    ...
    slots_nocturnos AS (
      SELECT 
        ...
        CASE 
          WHEN icm.ventana_inicio >= icm.ventana_fin THEN
            -- Primera parte: desde ventana_inicio hasta 23:59
            generate_series(
          ...
          ELSE
            -- Para ventanas diurnas, generar serie normal
            generate_series(
              date_trunc('day', now()) + icm.ventana_inicio,
      ...
      SELECT 
        ...
        icm.mensaje_template,
        icm.tenant_id,
        -- Segunda parte: desde 00:00 hasta ventana_fin
        generate_series(
