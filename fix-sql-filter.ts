...
async function fixSqlFilter() {
  console.log('🔧 CORRIGIENDO FILTRO SQL PARA MOSTRAR TURNOS ASISTIDOS\n');

  try {
    console.log('1️⃣ Actualizando vista para incluir estado "trabajado"...');
    
    await pool.query(`
      -- CORRECCIÓN CRÍTICA: Incluir estado 'trabajado' en el filtro de la vista
      -- Esto permite que los turnos asistidos aparezcan en la pauta diaria
      ...
      WITH pauta_dedup AS (
        ...
        WHERE po.activo = true
          AND pm.estado IN ('planificado', 'libre', 'sin_cobertura', 'trabajado')  -- ← CORREGIDO: Incluir 'trabajado'
      ...
      SELECT 
        ...
        CASE 
          ...
          WHEN pd.estado_ui = 'trabajado' THEN 'asistido'  -- ← CORREGIDO: 'trabajado' → 'asistido'
    ...
    result.rows.forEach((row: any, i: number) => {
      console.log(`${i+1}. ${row.instalacion_nombre} - ${row.puesto_nombre}`);
      ...
      console.log(`   Estado pauta mensual: "${row.estado_pauta_mensual}"`);
    ...
    console.log('\n🎯 RESULTADO ESPERADO:');
    console.log('- Los turnos asistidos ahora aparecerán en la pauta diaria');
    console.log('- El nombre de Astorga se mostrará como "Marcelo Astorga"');
    console.log('- Los botones de acciones estarán disponibles');
...
if (require.main === module) {
  fixSqlFilter().then(() => process.exit(0));
}
