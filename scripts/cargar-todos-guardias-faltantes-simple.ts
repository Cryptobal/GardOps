  ...
  try {
    ...
    for (const guardia of guardiasValidos) {
      try {
        ...
        // Coordenadas aproximadas por ciudad (versión simplificada)
        ...
        if (guardia.ciudad) {
          ...
          for (const [ciudad, coords] of Object.entries(coordenadasPorCiudad)) {
            if (ciudadKey.includes(ciudad.toLowerCase()) || 
                (guardia.comuna && guardia.comuna.toLowerCase().includes(ciudad.toLowerCase()))) {
              latitud = coords[0];
              longitud = coords[1];
              break;
            }
        ...
        // Si no se encontraron coordenadas, usar coordenadas de Santiago como fallback
