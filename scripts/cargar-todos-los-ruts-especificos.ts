...
// Datos de coordenadas por dirección (extracto del archivo latlng_guardias.csv)
...
function generarEmailUnico(rut: string, nombre: string, apellido: string): string {
  const nombreLimpio = nombre.toLowerCase().replace(/[^a-z]/g, '');
  const apellidoLimpio = apellido.toLowerCase().replace(/[^a-z]/g, '');
  const rutLimpio = rut.replace(/[^0-9]/g, '');
  
  return `${nombreLimpio}.${apellidoLimpio}.${rutLimpio}@gardops.cl`;
}
...
async function cargarGuardia(guardia: any) {
  ...
  const email = guardia.email || generarEmailUnico(guardia.rut, guardia.nombre, guardia.apellido_paterno);
  ...
  try {
    await query(sqlQuery, [
      id, tenantId, guardia.nombre || '', email, guardia.telefono || '', 
...
// Función para leer el CSV y filtrar por RUTs específicos
async function leerCSVYFiltrar(): Promise<any[]> {
...
// Función principal
async function cargarTodosLosRutsEspecificos() {
  console.log('🚀 Iniciando carga de todos los RUTs específicos...');
  console.log(`📋 RUTs a procesar: ${rutsEspecificos.length}`);
  
  try {
    // Leer CSV y filtrar
    const guardiasEncontrados = await leerCSVYFiltrar();
    
    if (guardiasEncontrados.length === 0) {
      console.log('❌ No se encontraron guardias en el CSV para los RUTs especificados');
      return;
    }
    ...
    for (const guardia of guardiasEncontrados) {
    ...
    console.log(`📊 Total procesados: ${guardiasEncontrados.length}`);
    
    return { exitosos, fallidos, total: guardiasEncontrados.length };
...
export { cargarTodosLosRutsEspecificos }; 