-- =====================================================
-- SCRIPT COMPLETO DE CORRECCIONES - GARDOPS
...
-- 2. Este script corrige el resto de problemas identificados
...
-- Verificar y corregir timestamps en tipos_documentos
DO $$
BEGIN
    -- Verificar si existen columnas con nombres en español
    IF EXISTS (
        ...
        RAISE NOTICE '✅ Renombrado creado_en → created_at en tipos_documentos';
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE tipos_documentos RENAME COLUMN modificado_en TO updated_at;
        RAISE NOTICE '✅ Renombrado modificado_en → updated_at en tipos_documentos';
    END IF;
END $$;

-- Verificar y corregir timestamps en planillas
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE '✅ Renombrado creado_en → created_at en planillas';
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE planillas RENAME COLUMN modificado_en TO updated_at;
        RAISE NOTICE '✅ Renombrado modificado_en → updated_at en planillas';
    END IF;
END $$;

-- Verificar y corregir timestamps en turnos_extras
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE '✅ Renombrado creado_en → created_at en turnos_extras';
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE turnos_extras RENAME COLUMN modificado_en TO updated_at;
        RAISE NOTICE '✅ Renombrado modificado_en → updated_at en turnos_extras';
    END IF;
END $$;

-- =====================================================
-- 2. CREACIÓN DE ÍNDICES FALTANTES
-- =====================================================

-- Índices para fechas de vencimiento
CREATE INDEX IF NOT EXISTS idx_documentos_fecha_vencimiento 
ON documentos_clientes (fecha_vencimiento);

CREATE INDEX IF NOT EXISTS idx_documentos_instalacion_fecha_vencimiento 
ON documentos_instalacion (fecha_vencimiento);
...
-- =====================================================
-- 3. VERIFICACIÓN Y CORRECCIÓN DE DOCUMENTOS_INSTALACION
-- =====================================================
...
BEGIN
    ...
    IF table_exists THEN
        ...
        IF record_count = 0 THEN
            ...
            IF tipo_doc_id IS NOT NULL THEN
            ...
            ELSE
                RAISE NOTICE '⚠️  No se encontraron tipos de documentos para insertar documento de prueba';
            END IF;
        END IF;
    ELSE
