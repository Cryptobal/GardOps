-- ===============================================
-- CORRECCIÓN FINAL DE FUNCIÓN fn_deshacer
-- ===============================================
-- Corrige el error "column reference 'pauta_id' is ambiguous" en el RETURN QUERY
...
BEGIN
    ...
    -- CORREGIDO: Usar p_pauta_id explícitamente en el RETURN QUERY
    RETURN QUERY SELECT TRUE, p_pauta_id, 'planificado'::text;
END;
$$;

-- 3. Comentario en la función
COMMENT ON FUNCTION as_turnos.fn_deshacer(bigint, text) IS 'Función para deshacer marcados de turnos - CORREGIDA: resuelve ambigüedad en RETURN QUERY';

-- 4. Probar la función
SELECT 'Probando función corregida' as test;
SELECT * FROM as_turnos.fn_deshacer(7003, 'test');
