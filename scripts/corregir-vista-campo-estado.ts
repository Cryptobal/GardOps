...
async function corregirVistaCampoEstado() {
  console.log('🔧 CORRIGIENDO VISTA - Campo estado correcto\n');

  try {
    console.log('1️⃣ Actualizando vista central_v_llamados_automaticos con pm.estado = planificado...');
    ...
    console.log('✅ Vista actualizada correctamente con pm.estado = planificado');
    
    // Verificar que la vista funciona
    console.log('\n2️⃣ Verificando que la vista funciona...');
    ...
    console.log(`📊 Llamados para hoy: ${result.rows[0].total_llamados}`);
    
    if (result.rows[0].total_llamados > 0) {
      console.log('✅ ¡Vista funcionando correctamente!');
    } else {
      console.log('⚠️ No hay llamados para hoy. Verificando configuración...');
      ...
      const configResult = await sql`
        ...
        WHERE i.nombre ILIKE '%bodega%'
      `;
      
      console.log('🔍 Configuración de Bodega Santa Amalia:');
