-- ===============================================
-- CORRECCIÓN FINAL DE VISTA PARA "SIN COBERTURA"
-- ===============================================
-- El problema: PPCs marcados como 'sin_cobertura' aparecen como 'planificado'
-- La solución: Priorizar el estado de la pauta mensual cuando es 'sin_cobertura'
...
SELECT 
  ...
  -- LÓGICA CORREGIDA DE ESTADO UI - PRIORIZAR ESTADO DE PAUTA MENSUAL
  CASE 
    -- Si es día libre
    WHEN pm.tipo_turno = 'libre' THEN 'libre'
    
    -- CORRECCIÓN CRÍTICA: Si el estado de la pauta mensual es 'sin_cobertura', respetarlo SIEMPRE
    WHEN pm.estado = 'sin_cobertura' THEN 'sin_cobertura'
    ...
    WHEN pm.estado_puesto = 'ppc' THEN
      CASE 
        ...
        -- PPC sin cobertura es planificado (solo si no está marcado como sin_cobertura)
...
WHERE po.activo = true
  AND pm.estado IN ('planificado', 'libre', 'sin_cobertura')  -- Incluir sin_cobertura
...
COMMENT ON VIEW public.as_turnos_v_pauta_diaria_unificada IS 'Vista unificada de pauta diaria - CORREGIDA: prioriza estado sin_cobertura de pauta mensual';
COMMENT ON VIEW public.as_turnos_v_pauta_diaria_dedup_fixed IS 'Vista deduplicada de pauta diaria - CORREGIDA: prioriza estado sin_cobertura de pauta mensual';
