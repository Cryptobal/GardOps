  ...
  try {
    ...
    if (conAsignaciones.rows.length > 0) {
      ...
      conAsignaciones.rows.forEach((g, i) => {
        ...
        console.log(`     Estado: ${g.estado}, Inicio: ${g.fecha_inicio}, Fin: ${g.fecha_fin}`);
      });
    } else {
    ...
    // 4. Verificar si hay asignaciones no activas que podrían estar afectando
    ...
    if (todasAsignaciones.rows.length > 0) {
      ...
      todasAsignaciones.rows.forEach((a, i) => {
        console.log(`  ${i+1}. ${a.nombre_completo} - ${a.estado} - ${a.rol_nombre} en ${a.instalacion_nombre}`);
        console.log(`     Fechas: ${a.fecha_inicio} → ${a.fecha_fin}`);
      });
    } else {
    ...
    console.log(`- Esperados disponibles: ${todosTest.rows.length - conAsignaciones.rows.length}`);
