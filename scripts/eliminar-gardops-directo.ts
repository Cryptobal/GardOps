...
async function eliminarGardOpsDirecto() {
  console.log('🗑️ Eliminando tenant GardOps directamente...\n');
  ...
  try {
    ...
    const { rowCount } = await sql`DELETE FROM tenants WHERE nombre = 'GardOps'`;
    console.log(`✅ Tenant GardOps eliminado (${rowCount} filas afectadas)`);
...
eliminarGardOpsDirecto().then(() => process.exit(0)).catch(console.error);
