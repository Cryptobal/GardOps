...
async function fixVistaDesdeCero() {
  console.log('🔧 Creando vista desde cero con lógica simple\n');
  ...
  try {
    ...
    // 2. Crear vista completamente nueva y simple
    console.log('2️⃣ Creando vista desde cero...');
    await pool.query(`
      CREATE VIEW central_v_llamados_automaticos AS
      WITH slots_base AS (
        SELECT 
          -- ID único basado en instalación, fecha y hora
          md5(concat(
            i.id::text, '|',
            pm.anio::text, '-', 
        ...
        CROSS JOIN LATERAL generate_series(
          ...
          -- Fecha del turno + 1 día + ventana_fin (07:00 del día siguiente)
    ...
    `);
    console.log('✅ Vista desde cero creada');
    ...
    console.log('\n✅ Vista desde cero creada exitosamente');
