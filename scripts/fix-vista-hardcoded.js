...
async function fixVistaHardcoded() {
  console.log('🔧 Creando vista con slots hardcodeados\n');
  ...
  try {
    ...
    // 2. Crear vista con slots hardcodeados
    console.log('2️⃣ Creando vista con slots hardcodeados...');
    await pool.query(`
      CREATE VIEW central_v_llamados_automaticos AS
      WITH slots_hardcoded AS (
        SELECT 
          -- ID único
          md5(concat(
            i.id::text, '|',
    ...
    `);
    console.log('✅ Vista con slots hardcodeados creada');
    ...
    console.log('\n✅ Vista con slots hardcodeados creada exitosamente');
