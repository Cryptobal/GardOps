...
async function fixVistaHorasCorrectas() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Creando vista con horas correctas');
    ...
    // 2. Crear vista con horas correctas
    console.log('2️⃣ Creando vista con horas correctas...');
    await client.query(`
      ...
      WITH slots_base AS (
        SELECT
          ...
          pm.id as pauta_id,
          po.id as puesto_id,
          -- Crear timestamp correcto
          ((pm.anio || '-' || 
            LPAD(pm.mes::text, 2, '0') || '-' || 
            LPAD(pm.dia::text, 2, '0') || ' ' || 
            LPAD(hora_slot::text, 2, '0') || ':00:00')::timestamp AT TIME ZONE 'America/Santiago') AT TIME ZONE 'UTC' as programado_para,
          ...
          CASE 
            WHEN ((pm.anio || '-' || 
                 ...
                 (now() AT TIME ZONE 'America/Santiago') - interval '30 minutes'
            THEN true ELSE false 
          END as es_urgente,
          CASE 
            WHEN date_trunc('hour', ((pm.anio || '-' || 
                   LPAD(pm.mes::text, 2, '0') || '-' || 
                   LPAD(pm.dia::text, 2, '0') || ' ' || 
                   LPAD(hora_slot::text, 2, '0') || ':00:00')::timestamp AT TIME ZONE 'America/Santiago')) = 
    ...
    `);
    console.log('✅ Vista con horas correctas creada');
    ...
    console.log('✅ Vista con horas correctas creada exitosamente');
...
fixVistaHorasCorrectas().catch(console.error);
