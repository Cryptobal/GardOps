...
async function fixVistaSimple() {
  console.log('🔧 Creando vista simple y correcta\n');
  ...
  try {
    ...
    // 2. Crear vista simple y directa
    console.log('2️⃣ Creando vista simple...');
    await pool.query(`
      CREATE VIEW central_v_llamados_automaticos AS
      WITH slots_base AS (
        SELECT 
          -- Generar slots exactos para cada instalación con turnos planificados
          md5(concat(
            i.id::text, '|',
            pm.anio::text, '-', 
    ...
    console.log('\n✅ Vista simple creada exitosamente');
