...
async function fixVistaTimestampsCorrectos() {
  const client = await pool.connect();
  
  try {
    console.log('🔧 Creando vista con timestamps correctos');
    ...
    // 2. Crear vista con timestamps correctos
    console.log('2️⃣ Creando vista con timestamps correctos...');
    await client.query(`
      ...
      WITH slots_base AS (
        ...
        CROSS JOIN LATERAL (
          ...
          SELECT timestamp_final FROM (
            ...
            -- Primero crear el timestamp en Chile, luego convertir a UTC
            SELECT timezone('UTC', timezone('America/Santiago', (pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0') || ' 19:00:00')::timestamp)) as timestamp_final
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', (pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0') || ' 20:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', (pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0') || ' 21:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', (pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0') || ' 22:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', (pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0') || ' 23:00:00')::timestamp))
            UNION ALL
            -- Horas del día siguiente: 00:00, 01:00, 02:00, 03:00, 04:00, 05:00, 06:00, 07:00
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '00:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '01:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '02:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '03:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '04:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '05:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '06:00:00')::timestamp))
            UNION ALL
            SELECT timezone('UTC', timezone('America/Santiago', ((pm.anio || '-' || LPAD(pm.mes::text, 2, '0') || '-' || LPAD(pm.dia::text, 2, '0'))::date + interval '1 day' + time '07:00:00')::timestamp))
          ) timestamps
        ) slots
    ...
    `);
    console.log('✅ Vista con timestamps correctos creada');
    ...
    console.log('✅ Vista con timestamps correctos creada exitosamente');
