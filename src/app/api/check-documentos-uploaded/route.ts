...
export async function GET(req: NextRequest) {
  try {
    logger.debug("🔍 Verificando documentos subidos recientemente...");
    ...
    logger.debug(`📊 Documentos encontrados: ${documentos.rows.length}`);
    documentos.rows.forEach((doc: any) => {
      logger.debug(`  - ID: ${doc.id}, Tipo: ${doc.tipo}, Nombre: ${doc.nombre_original}, Guardia: ${doc.guardia_id}`);
    });
    ...
    logger.debug(`📊 Documentos del guardia de prueba: ${guardiaDocs.rows.length}`);
    guardiaDocs.rows.forEach((doc: any) => {
      logger.debug(`  - ID: ${doc.id}, Tipo: ${doc.tipo}, Nombre: ${doc.nombre_original}`);
    });
