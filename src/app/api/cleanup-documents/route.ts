...
export async function POST(req: NextRequest) {
  try {
    logger.debug('🧹 Iniciando limpieza automática de documentos...');
    ...
    try {
      ...
      logger.debug(`📋 Encontrados ${documentos.length} documentos en la base de datos`);
        ...
        try {
          ...
          try {
            ...
            if (documentosVerificados % 10 === 0) {
              logger.debug(`✅ Verificados ${documentosVerificados}/${documentos.length} documentos`);
            }
            ...
            if (r2Error.name === 'NotFound' || r2Error.$metadata?.httpStatusCode === 404) {
              ...
              logger.debug(`✅ Eliminado documento fantasma: ${doc.nombre_original}`);
            } else {
      ...
      if (documentosSinUrl.rows.length > 0) {
        logger.debug(`🗑️ Eliminados ${documentosSinUrl.rows.length} documentos sin URL válida`);
      ...
      for (const ref of referenciasInvalidas.rows) {
        if (ref.documentos_huérfanos > 0) {
          logger.debug(`⚠️ ${ref.documentos_huérfanos} documentos huérfanos en módulo ${ref.modulo}`);
      ...
      if (documentosHuérfanos > 0) {
        ...
        logger.debug(`🗑️ Eliminados ${documentosHuérfanos} documentos huérfanos`);
      ...
      logger.debug(`   - Con URL válida: ${stats.con_url}`);
...
export async function GET(req: NextRequest) {
  try {
    logger.debug('🔍 Verificando estado de documentos...');
