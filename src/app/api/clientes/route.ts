...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      if (!email) {
        logger.debug('❌ No se pudo obtener email del usuario');
        return NextResponse.json({ ok: false, error: 'no-auth' }, { status: 401 });
      }

      devLogger.search(' Verificando permisos para:', email);
      ...
      if (userCheck.rows.length === 0) {
        logger.debug('❌ Usuario no encontrado en BD:', email);
        return NextResponse.json({ ok: false, error: 'user-not-found' }, { status: 403 });
      }

      const user = userCheck.rows[0];
      devLogger.success(' Usuario encontrado:', { id: user.id });
      ...
      try {
        ...
        devLogger.search(' Verificación de permiso clientes.view:', hasPermission);
        
        if (!hasPermission) {
          logger.debug('❌ Usuario no tiene permiso clientes.view');
      ...
      logger.debug('✅ Permisos verificados, obteniendo clientes...');
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.debug('⚠️ Modo desarrollo: permitiendo acceso por fallo en verificación');
        const clientes = await obtenerClientes();
        return NextResponse.json({ success: true, data: clientes });
      }
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    devLogger.process(' API Clientes - Actualizando cliente:', body);
...
async function ensureClientesTable() {
  try {
    ...
    if (!checkTable.rows[0].exists) {
      logger.debug('📊 Creando tabla clientes...');
      ...
      logger.debug('✅ Tabla clientes creada exitosamente');
    }
  } catch (error) {
