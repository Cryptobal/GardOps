...
export async function GET(req: NextRequest) {
  try {
    ...
    devLogger.search(' Debug Clientes Auth - Email recibido:', email);
    ...
    try {
      ...
      devLogger.search(' Buscando usuario en BD con email:', email);
      
      const userCheck = await sql`
        SELECT id, email, rol, activo FROM public.usuarios WHERE lower(email) = lower(${email}) LIMIT 1
      `;

      devLogger.search(' Resultado de búsqueda en BD:', userCheck.rows);
