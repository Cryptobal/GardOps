...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    devLogger.search(' Query SQL:', sql);
    logger.debug('📋 Parámetros:', params);

    logger.debug('🔍 Ejecutando query...');
    const result = await query(sql, params);
    devLogger.success(' Query ejecutada exitosamente, filas:', result.rows.length);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (s3 && documento.url && documento.url.includes('docs.gard.cl')) {
      try {
        logger.debug('🗑️ Eliminando archivo de R2:', documento.url);
        
        // Extraer la key del archivo de la URL
        const urlParts = documento.url.split('docs.gard.cl/');
        if (urlParts.length > 1) {
          const key = urlParts[1];
          logger.debug('🔑 Key del archivo en R2:', key);
          ...
          devLogger.success(' Archivo eliminado de R2:', key);
        }
      } catch (r2Error) {
    ...
    devLogger.success(' Documento eliminado de la BD:', documentId);
