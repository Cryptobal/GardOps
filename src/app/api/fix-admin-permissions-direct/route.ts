...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const permiso of permisosBasicos) {
      try {
        ...
        if (permExists.rows.length === 0) {
          ...
          `;
          console.log(`✅ Permiso ${permiso} creado`);
        }
      } catch (error) {
        console.log(`⚠️ Error con permiso ${permiso}:`, error);
      }
    }
    ...
    if (adminRole.rows.length === 0) {
      ...
      console.log('✅ Rol admin existente con ID:', adminRoleId);
    }
    ...
    for (const perm of allPermisos.rows) {
      try {
        ...
        if (alreadyAssigned.rows.length === 0) {
          ...
          `;
          console.log(`✅ Permiso ${perm.clave} asignado al rol admin`);
        } else {
          console.log(`✅ Permiso ${perm.clave} ya estaba asignado`);
        }
      } catch (error) {
        console.log(`⚠️ Error asignando permiso ${perm.clave}:`, error);
      }
    }
    
    // 6. Verificar que el usuario tenga el permiso clientes.view
    console.log('🔍 Verificando permiso clientes.view...');
    ...
    console.log('🔍 Resultado de verificación:', permCheck.rows);
    
    // 7. Crear función simple de verificación de permisos
    console.log('🔧 Creando función simple de verificación...');
    try {
      await sql`
        ...
        $$ LANGUAGE plpgsql;
      `;
      console.log('✅ Función simple de verificación creada');
    } catch (error) {
      console.log('⚠️ Error creando función simple:', error);
    }
    
    console.log('✅ Permisos del admin asignados DIRECTAMENTE');
