...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('📋 Creando vista v_usuarios_permisos...');
    try {
      await sql`
    ...
    if (adminUser.rows.length > 0) {
      ...
      if (admin.rol !== 'admin') {
        console.log('🔄 Actualizando rol del admin...');
        await sql`
          UPDATE usuarios SET rol = 'admin' WHERE email = 'carlos.irigoyen@gard.cl'
        `;
        console.log('✅ Rol del admin actualizado');
      }
          ...
          for (const perm of allPermisos.rows) {
            try {
              ...
              if (alreadyAssigned.rows.length === 0) {
                ...
                `;
                console.log(`✅ Permiso ${perm.id} asignado al rol admin`);
              }
            } catch (error) {
              console.log(`⚠️ Error asignando permiso ${perm.id}:`, error);
            }
          }
        }
      } catch (error) {
        console.log('❌ Error asignando permisos al rol:', error);
      }
    }
    
    console.log('✅ Sistema RBAC restaurado exitosamente');
