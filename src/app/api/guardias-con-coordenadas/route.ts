...
export async function GET(request: NextRequest) {
  logger.debug('🔍 API Guardias - Iniciando request');
  ...
  if (authResult instanceof NextResponse) {
    logger.debug('❌ API Guardias - Error de autenticación:', authResult.status);
    return authResult;
  }
  ...
  if (!user) {
    logger.debug('❌ API Guardias - Usuario no encontrado');
  ...
  devLogger.success(' API Guardias - Usuario autenticado:', user.email, 'Tenant:', user.tenant_id);
  try {
    // Usar el tenant_id del usuario autenticado
    const tenantId = user.tenant_id;
    devLogger.search(' API Guardias - Ejecutando query con tenant_id:', tenantId);
    ...
    devLogger.success(' API Guardias - Query ejecutada, filas encontradas:', result.rows.length);
    ...
    devLogger.success(' API Guardias - Mapeo completado, guardias procesados:', guardias.length);
