...
export async function GET(
  ...
  try {
    ...
    logger.debug(`🔍 Consultando historial mensual para guardia ${guardiaId}, mes ${mesActual}, año ${anioActual}`);
    ...
    if (guardiaResult.rows.length === 0) {
      logger.debug(`❌ Guardia ${guardiaId} no encontrado`);
      return NextResponse.json(
    ...
    logger.debug(`✅ Historial mensual cargado correctamente para el guardia ${guardia.nombre} ${guardia.apellido_paterno}`);
    logger.debug(`📊 Registros encontrados: ${historial.length}`);

    if (historial.length === 0) {
      logger.debug(`ℹ️ No hay registros para el guardia ${guardiaId} en ${mesActual}/${anioActual}`);
    }
