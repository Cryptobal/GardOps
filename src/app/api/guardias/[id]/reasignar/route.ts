  ...
  try {
    ...
    try {
      ...
      if (asignacionActual.rows.length > 0) {
        ...
        logger.debug(`✅ Asignación anterior cerrada: ${asignacion.instalacion_nombre}`);
      }
      ...
      if (ppcActivo.rows.length > 0) {
        ...
        logger.debug(`✅ PPC anterior cerrado: ${ppc.instalacion_nombre}`);
      }
      ...
      logger.debug('✅ Reasignación completada exitosamente');
...
export async function GET(
  ...
  try {
    ...
    logger.error('Error obteniendo información de reasignación::', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
