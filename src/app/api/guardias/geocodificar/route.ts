...
export async function POST(request: NextRequest) {
  try {
    console.log('🗺️ Iniciando geocodificación masiva de guardias...');
    ...
    for (let i = 0; i < resultados.length; i++) {
      ...
      if (resultado) {
        try {
          ...
          actualizados++;
          console.log(`✅ ${guardia.nombre} ${guardia.apellido_paterno}: coordenadas actualizadas`);
        } catch (error) {
    ...
    console.log('✅ Geocodificación masiva completada:', response);
...
export async function GET(request: NextRequest) {
  try {
    console.log('📊 Obteniendo estadísticas de geocodificación...');
    ...
    console.log('✅ Estadísticas de geocodificación obtenidas');
    return NextResponse.json(response);
