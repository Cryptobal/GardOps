...
function convertirFecha(fecha: any): string | null {
  ...
  try {
    ...
    logger.debug(`⚠️ No se pudo convertir fecha: ${fechaStr}`);
    return null;
  } catch (error) {
    logger.debug(`Error convirtiendo fecha: ${fechaStr}`, error);
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🚀 Importando guardias desde Excel...');
    ...
    logger.debug(`📊 Procesando ${excelData.length} filas del Excel...`);
    ...
    for (let i = 0; i < excelData.length; i++) {
      ...
      try {
        ...
        if (isNewGuardia) {
          // CREAR NUEVO GUARDIA
          logger.debug(`🆕 Creando nuevo guardia en fila ${rowNumber}...`);
            ...
            if (row[excelField] !== undefined && row[excelField] !== null && row[excelField] !== '') {
              ...
              if (excelField === 'Activo') {
                ...
                if (fechaConvertida) {
                  ...
                  logger.debug(`✅ Fecha Nacimiento convertida: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Nacimiento inválida en fila ${rowNumber}: ${value}`);
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha OS10 convertida: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha OS10 inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha Ingreso convertida: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Ingreso inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha Finiquito convertida: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Finiquito inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (!isNaN(numValue) && numValue >= 0 && numValue <= 999999) {
                  ...
                  logger.debug(`⚠️ Monto anticipo inválido en fila ${rowNumber}: ${value}`);
                ...
                if (!isNaN(numValue) && numValue >= 0) {
                  ...
                  logger.debug(`⚠️ Días vacaciones inválidos en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si es inválido
                }
                ...
                try {
                  ...
                  if (!nombreFinal) {
                    logger.debug(`⚠️ Banco vacío en fila ${rowNumber}, omitiendo campo banco`);
                  ...
                  logger.debug(`⚠️ Error buscando banco en fila ${rowNumber}: ${bancoNombre}`, error);
                ...
                if (['cct', 'cuenta corriente'].includes(tipoCuentaRaw)) {
                  ...
                  logger.debug(`⚠️ Tipo de cuenta inválido en fila ${rowNumber}: ${value}. Usando CTE por defecto.`);
          ...
          logger.debug(`✅ Nuevo guardia creado correctamente en fila ${rowNumber}`);
          ...
          logger.debug(`🔄 Actualizando guardia existente ${guardiaId}...`);
        ...
        const fieldMappings: { [key: string]: string } = {
          ...
          for (const [excelField, dbField] of Object.entries(fieldMappings)) {
            if (row[excelField] !== undefined && row[excelField] !== null && row[excelField] !== '') {
              ...
              if (excelField === 'Activo') {
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha Nacimiento convertida para actualización: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Nacimiento inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha OS10 convertida para actualización: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha OS10 inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha Ingreso convertida para actualización: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Ingreso inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                if (fechaConvertida) {
                  value = fechaConvertida;
                  logger.debug(`✅ Fecha Finiquito convertida para actualización: ${row[excelField]} -> ${value}`);
                } else {
                  logger.debug(`⚠️ Fecha Finiquito inválida en fila ${rowNumber}: ${value}`);
                  continue; // Saltar este campo si la fecha es inválida
                }
                ...
                try {
                  ...
                  if (bancoResult.rows.length > 0) {
                    value = bancoResult.rows[0].id;
                    logger.debug(`✅ Banco encontrado para actualización: ${bancoNombre} -> ${value}`);
                  } else {
                    logger.debug(`⚠️ Banco no encontrado en fila ${rowNumber}: ${bancoNombre}`);
                    continue; // Saltar este campo si el banco no existe
                  }
                } catch (error) {
                  logger.debug(`⚠️ Error buscando banco en fila ${rowNumber}: ${bancoNombre}`, error);
          ...
          if (updateFields.length > 1) { // > 1 porque siempre agregamos updated_at
            ...
            logger.debug(`✅ Guardia ${guardiaId} actualizado correctamente`);
          }
        }
    ...
    logger.debug(`📊 Importación completada: ${creados} creados, ${actualizados} actualizados, ${errores} errores`);
