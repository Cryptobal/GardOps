...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.debug(`🔍 Obteniendo KPIs de OS10 con ${diasAlerta} días de alerta`);
    ...
    logger.debug(`📊 Total guardias activos: ${guardias.length}`);

    // Calcular KPIs usando la función utilitaria
    const kpisOS10 = obtenerKPIsOS10(guardias, diasAlerta);

    logger.debug('📈 KPIs OS10 calculados:', kpisOS10);
