...
export async function GET(request: NextRequest) {
  logger.debug('🔍 API Instalaciones - Iniciando request');
  ...
  if (authResult instanceof NextResponse) {
    logger.debug('❌ API Instalaciones - Error de autenticación:', authResult.status);
    return authResult;
  }
  ...
  if (!user) {
    logger.debug('❌ API Instalaciones - Usuario no encontrado');
  ...
  devLogger.success(' API Instalaciones - Usuario autenticado:', user.email, 'Tenant:', user.tenant_id);
  try {
