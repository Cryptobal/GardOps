...
export async function GET(
  ...
  try {
    ...
    const activoFlag = !(activoParam === 'false');
    ...
    logger.error('Error obteniendo estructuras de servicio::', error);
...
export async function POST(
  ...
  try {
    ...
    if (nombre_bono === 'Sueldo Base') {
      // 1) ¿Existe base ACTIVA?
      const baseActiva = await query(
        `SELECT id FROM sueldo_estructuras_servicio 
         WHERE instalacion_id = $1 AND rol_servicio_id = $2 AND bono_id IS NULL AND activo = true
         LIMIT 1`,
        [instalacionId, rol_servicio_id]
      );
      const baseActivaRow = Array.isArray(baseActiva) ? baseActiva[0] : (baseActiva.rows || [])[0];
      if (baseActivaRow) {
        // Si hay activa, solo actualizamos su monto
        const upd = await query(
          ...
          [monto, baseActivaRow.id]
        );
        const r = Array.isArray(upd) ? upd[0] : (upd.rows || [])[0];
        return NextResponse.json({ ...r, nombre_bono: 'Sueldo Base', monto, imponible: true });
      }

      // 2) ¿Existe base INACTIVA? → crear una NUEVA activa y dejar la otra intacta
      const baseCualquiera = await query(
        `SELECT id FROM sueldo_estructuras_servicio 
         WHERE instalacion_id = $1 AND rol_servicio_id = $2 AND bono_id IS NULL
         LIMIT 1`,
        [instalacionId, rol_servicio_id]
      );
      const baseRow = Array.isArray(baseCualquiera) ? baseCualquiera[0] : (baseCualquiera.rows || [])[0];
      const ins = await query(
        `INSERT INTO sueldo_estructuras_servicio (
           instalacion_id, rol_servicio_id, sueldo_base, bono_id, monto, activo, fecha_inactivacion
         ) VALUES ($1, $2, $3, NULL, 0, true, NULL)
    ...
    logger.error('Error creando estructura de servicio::', error);
    return NextResponse.json(
