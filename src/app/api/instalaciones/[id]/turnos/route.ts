...
export async function GET(
  ...
  logger.debug("🔁 Endpoint activo: /api/instalaciones/[id]/turnos");
...
export async function POST(
  ...
  try {
    ...
    logger.debug(`✅ Turno creado con ${puestosCreados.rows.length} puestos operativos`);
    ...
    logger.error('Error creando turno de instalación::', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
