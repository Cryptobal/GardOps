...
export async function GET(
  ...
  logger.debug("🔁 Endpoint activo: /api/instalaciones/[id]/turnos_v2");
  
  try {
    const instalacionId = params.id;
    logger.debug("🔍 Buscando turnos para instalación:", instalacionId);
    ...
    logger.debug("✅ Turnos procesados:", turnos.length);
...
export async function POST(
  ...
  logger.debug("🔁 Endpoint activo: /api/instalaciones/[id]/turnos_v2 POST");
  ...
  try {
    ...
    logger.error('Error creando turno v2::', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
