...
export async function POST() {
  try {
    console.log('🤖 Iniciando asignación automática de PPCs...');
    ...
    for (const ppc of ppcsPendientes.rows) {
      ...
      if (candidatosDisponibles.length === 0) {
        console.log(`⚠️ PPC ${ppc.id}: Sin candidatos disponibles`);
        continue;
      }
    ...
    console.log(`🎯 Asignación automática completada: ${asignacionesRealizadas} asignaciones realizadas`);
