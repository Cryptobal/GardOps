...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🚀 Importando instalaciones desde Excel...');
    ...
    for (let i = 0; i < excelData.length; i++) {
      ...
      try {
        ...
        if (isNewInstalacion) {
          ...
          logger.debug(`🆕 Creando nueva instalación en fila ${rowNumber}...`);
          ...
          creados++;
          logger.debug(`✅ Instalación creada con ID: ${insertResult.rows[0].id}`);

        } else {
          // ACTUALIZAR INSTALACIÓN EXISTENTE
          logger.debug(`📝 Actualizando instalación existente: ${instalacionId}`);
          ...
          if (updateFields.length > 0) {
            ...
            actualizados++;
            logger.debug(`✅ Instalación actualizada: ${instalacionId}`);
          } else {
            logger.debug(`⚠️ No hay cambios para la instalación: ${instalacionId}`);
          }
        }
    ...
    devLogger.success(' Importación de instalaciones completada:', response);
