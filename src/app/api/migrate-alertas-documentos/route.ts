...
export async function GET(request: NextRequest) {
  try {
    logger.debug('🚀 Iniciando migración completa de sistema de vencimientos y alertas documentales...');

    // 1. Modificar tabla tipos_documentos
    logger.debug('📋 Agregando campos de vencimiento a tipos_documentos...');
    ...
    if (!existingColumns.includes('requiere_vencimiento')) {
      ...
      `);
      logger.debug('✅ Columna requiere_vencimiento agregada');
    }
    ...
    if (!existingColumns.includes('dias_antes_alarma')) {
      ...
      `);
      logger.debug('✅ Columna dias_antes_alarma agregada');
    }

    // 2. Modificar tabla documentos
    logger.debug('📋 Agregando campo fecha_vencimiento a documentos...');
    ...
    if (docColumnCheck.rows.length === 0) {
      ...
      `);
      logger.debug('✅ Columna fecha_vencimiento agregada a documentos');
    }

    // 3. Crear tabla alertas_documentos
    logger.debug('📋 Creando tabla alertas_documentos...');
    await query(`
    ...
    logger.debug('✅ Tabla alertas_documentos creada con índices');

    // 4. Verificar estructura final
    logger.debug('📋 Verificando estructura de tipos_documentos:');
    ...
    logger.debug('📋 Verificando estructura de documentos:');
    ...
    logger.debug('📋 Verificando estructura de alertas_documentos:');
    ...
    logger.debug('🎉 Migración completa del sistema de vencimientos y alertas documentales completada exitosamente!');
