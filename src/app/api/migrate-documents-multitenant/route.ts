...
export async function GET(req: NextRequest) {
  try {
    console.log('🔧 Iniciando migración de documentos a multitenant...');
    ...
    try {
      ...
      console.log('2️⃣ Agregando tenant_id a tipos_documentos...');
      ...
      if (!tiposColumnCheck.rows[0].exists) {
        ...
        `, [tenantId]);
        console.log('✅ Registros existentes actualizados con tenant_id');
      } else {
        console.log('ℹ️ Columna tenant_id ya existe en tipos_documentos');
      }

      // 3. AGREGAR TENANT_ID A DOCUMENTOS
      console.log('3️⃣ Agregando tenant_id a documentos...');
      ...
      if (!docsColumnCheck.rows[0].exists) {
        ...
        `);
        console.log('✅ Columna tenant_id agregada a documentos');
        ...
        console.log('ℹ️ Columna tenant_id ya existe en documentos');
      }

      // 4. AGREGAR TENANT_ID A ALERTAS_DOCUMENTOS
      console.log('4️⃣ Agregando tenant_id a alertas_documentos...');
      ...
      if (!alertasColumnCheck.rows[0].exists) {
        ...
        `);
        console.log('✅ Columna tenant_id agregada a alertas_documentos');
        ...
        console.log('ℹ️ Columna tenant_id ya existe en alertas_documentos');
      }

      // 5. RENOMBRAR TIPOS_DOCUMENTOS A DOCUMENTOS_TIPOS
      console.log('5️⃣ Renombrando tipos_documentos a documentos_tipos...');
      ...
      if (tiposTableCheck.rows[0].exists) {
        ...
        `);
        console.log('✅ Tabla tipos_documentos renombrada a documentos_tipos');
        ...
        try {
          ...
          `);
          console.log('✅ Índices renombrados');
        } catch (error) {
          console.log('ℹ️ No se pudieron renombrar algunos índices (puede que no existan)');
        }
      } else {
        console.log('ℹ️ Tabla tipos_documentos no existe, puede que ya haya sido renombrada');
      }

      // 6. CREAR ÍNDICES PARA TENANT_ID
      console.log('6️⃣ Creando índices para tenant_id...');
      ...
      console.log('✅ Índices de tenant_id creados');
      ...
      console.log('📋 Columnas tenant_id encontradas:');
      ...
      stats.rows.forEach((stat: any) => {
        console.log(`   - ${stat.tabla}: ${stat.con_tenant_id}/${stat.total_registros} registros con tenant_id`);
      });
