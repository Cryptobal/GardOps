...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🔄 Iniciando migración: Agregar columna codigo a TE_planillas_turnos_extras...');

    // 1. Agregar columna codigo si no existe
    logger.debug('📝 Agregando columna codigo...');
    await query(`
    ...
    logger.debug('📊 Creando índice para codigo...');
    await query(`
    ...
    logger.debug('🔄 Generando códigos para planillas existentes...');
    ...
    logger.debug(`📋 Encontradas ${planillasSinCodigo.length} planillas sin código`);
    ...
    logger.debug(`✅ ${totalActualizadas} planillas actualizadas con código`);
