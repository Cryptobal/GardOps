...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🔄 Iniciando migración: Renombrar tablas con prefijo TE_...');
    ...
    logger.debug('🔄 Renombrando planillas_turnos_extras...');
    try {
      await query('ALTER TABLE planillas_turnos_extras RENAME TO TE_planillas_turnos_extras');
      logger.debug('✅ planillas_turnos_extras → TE_planillas_turnos_extras');
    } catch (error) {
      logger.debug('⚠️ planillas_turnos_extras ya renombrada o no existe');
    }

    // 3. Renombrar turnos_extras a TE_turnos_extras
    logger.debug('🔄 Renombrando turnos_extras...');
    try {
      await query('ALTER TABLE turnos_extras RENAME TO TE_turnos_extras');
      logger.debug('✅ turnos_extras → TE_turnos_extras');
    } catch (error) {
      logger.debug('⚠️ turnos_extras ya renombrada o no existe');
    }

    // 4. Renombrar planilla_turno_relacion a TE_planilla_turno_relacion (si existe)
    logger.debug('🔄 Renombrando planilla_turno_relacion...');
    try {
      await query('ALTER TABLE planilla_turno_relacion RENAME TO TE_planilla_turno_relacion');
      logger.debug('✅ planilla_turno_relacion → TE_planilla_turno_relacion');
    } catch (error) {
      logger.debug('⚠️ planilla_turno_relacion ya renombrada o no existe');
    }

    // 5. Actualizar índices con nuevos nombres
    logger.debug('📊 Actualizando índices...');
    ...
    for (const indexQuery of indicesPlanillas) {
      try {
        ...
        logger.warn(' Índice ya existe o error:', error);
      }
    }
    ...
    devLogger.success(' Migración completada:', resultado);
