...
export async function GET(req: NextRequest) {
  try {
    logger.debug("🔧 Iniciando migración de tipos de documentos de postulación...");
    ...
    logger.debug("✅ Tabla tipos_documentos_postulacion creada/verificada");
    ...
    for (const tipo of tiposDefault) {
      try {
        ...
        logger.debug(`⚠️ Error insertando tipo ${tipo.nombre}:`, error);
      }
    }

    logger.debug("✅ Tipos de documentos de postulación insertados");
    ...
    logger.debug("📋 Tipos de documentos de postulación disponibles:");
