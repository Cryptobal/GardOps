...
export async function GET(request: NextRequest) {
  try {
    logger.debug('🚀 Iniciando migración de tipos de documentos...');

    // 1. Crear tabla tipos_documentos
    logger.debug('📋 Creando tabla tipos_documentos...');
    await query(`
    ...
    logger.debug('🔗 Verificando columna tipo_documento_id en documentos...');
    ...
    if (!columnExists.rows[0].exists) {
      logger.debug('➕ Agregando columna tipo_documento_id a documentos...');
      await query(`
    ...
    logger.debug('📝 Insertando tipos de documentos por defecto...');
    const tiposDefault = [
    ...
    for (const tipo of tiposDefault) {
      ...
      if (existe.rows.length === 0) {
        await query(`
          ...
          VALUES ($1, $2, true, NOW())
        `, [tipo.modulo, tipo.nombre]);
        logger.debug(`✅ Creado tipo: ${tipo.modulo} - ${tipo.nombre}`);
      } else {
        logger.debug(`⏭️ Tipo ya existe: ${tipo.modulo} - ${tipo.nombre}`);
      }
    }
    ...
    logger.debug(`📊 Total de tipos de documentos: ${tiposCount.rows[0].total}`);
