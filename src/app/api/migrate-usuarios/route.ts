...
export async function GET() {
  logger.debug('🚫 API /api/migrate-usuarios DESHABILITADA - Ya no recrea usuarios automáticamente');
  ...
  /*
  try {
    logger.debug('🔧 Iniciando migración completa de usuarios...');
    ...
    `);
    logger.debug('✅ Tabla tenants verificada');
    ...
    if (existingTenant.rows.length === 0) {
      ...
      logger.debug('✅ Tenant por defecto creado');
    } else {
      tenantId = existingTenant.rows[0].id;
      logger.debug('ℹ️ Tenant ya existe');
    }

    // 3. Eliminar tabla usuarios existente si tiene estructura incorrecta
    try {
      await query('DROP TABLE IF EXISTS usuarios CASCADE;');
      logger.debug('🗑️ Tabla usuarios anterior eliminada');
    } catch (error) {
      logger.debug('ℹ️ No había tabla usuarios anterior o error al eliminar:', error);
    }
    ...
    `);
    logger.debug('✅ Tabla usuarios creada con estructura correcta');
    ...
    logger.debug('🔑 CREDENCIALES DE ACCESO:');
    logger.debug('👑 Admin: carlos.irigoyen@gard.cl / admin123');
    logger.debug('👨‍💼 Supervisor: supervisor@gardops.com / super123');
    logger.debug('👮 Guardia: guardia@gardops.com / guard123');
    logger.debug(`🌐 Accede en: http://localhost:${process.env.PORT || 3001}/login`);
