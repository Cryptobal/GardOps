...
export async function GET(request: NextRequest) {
  try {
    ...
    logger.debug('📊 Dashboard - Período:', periodo, 'meses');
    ...
    devLogger.search(' Dashboard Query:', queryString);
    devLogger.search(' Dashboard Params:', params);

    const { rows } = await query(queryString, params);

    logger.debug('📊 Dashboard - Turnos obtenidos:', rows.length);
    if (rows.length > 0) {
