...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🔄 Iniciando sincronización de coberturas con turnos extras');
    ...
    logger.debug(`📊 Coberturas encontradas sin turno extra: ${coberturasPendientes.length}`);
    ...
    for (const cobertura of coberturasPendientes) {
      try {
        if (!cobertura.cobertura_guardia_id) {
          logger.debug(`⚠️ Saltando pauta ${cobertura.pauta_id}: sin guardia de cobertura`);
          continue;
        }
        ...
        if (existing.length > 0) {
          logger.debug(`⚠️ Turno extra ya existe para pauta ${cobertura.pauta_id}`);
          continue;
        }
        ...
        sincronizados++;
        logger.debug(`✅ Turno extra creado para pauta ${cobertura.pauta_id} - Fecha: ${cobertura.fecha}`);
