...
export async function POST(req: NextRequest) {
  ...
  logger.debug(`[${timestamp}] 🚀 Endpoint de guardado de pauta mensual ejecutándose`);
  ...
  try {
    ...
    if (body && typeof body === 'object' && 'instalacion_id' in body && ('pauta' in body || 'actualizaciones' in body)) {
      logger.debug('📝 Formato frontend detectado:', { 
      ...
      if (!Array.isArray(pauta) || pauta.length === 0) {
        logger.debug('❌ Pauta/actualizaciones no es un array válido:', { pauta, pautaType: typeof pauta });
      ...
      if (puestosResult.rows.length === 0) {
        logger.debug('❌ No se encontraron puestos operativos para instalación:', instalacion_id);
        return NextResponse.json({ error: 'No se encontraron puestos operativos para esta instalación' }, { status: 400 });
      }

      logger.debug('📋 Puestos operativos encontrados:', puestosResult.rows.length);

      // Generar turnos para cada día del mes
      const diasEnMes = new Date(anio, mes, 0).getDate();
      logger.debug('📅 Generando turnos para', diasEnMes, 'días del mes');
      ...
      logger.debug(`🔄 Convertidos ${pauta.length} guardias a ${turnos.length} turnos para ${diasEnMes} días`);
      ...
      logger.debug('📝 Formato directo detectado:', { turnos_count: body.length });
...
async function procesarTurnos(turnos: any[]) {
    ...
    try {
      ...
      if (estado === null || estado === '') {
        ...
        if (rowCount && rowCount > 0) {
          eliminados++;
          logger.debug(`🗑️ Eliminado turno para puesto ${puesto_id}, día ${dia}`);
        } else {
        ...
        if (guardia_id && estado === 'planificado') {
          try {
            ...
            logger.warn('⚠️ No se pudo pre-liberar duplicados guardia/día:', { turno, error: preCleanErr });
          }
        }
      ...
      if (pgErr && (pgErr.code === '23505' || (pgErr.message && pgErr.message.toLowerCase().includes('duplicate key')))) {
        try {
          ...
          if (guardia_id) {
            ...
            if (rowCount && rowCount > 0) {
              logger.debug(`♻️ Resuelto duplicado moviendo turno del guardia ${guardia_id} al puesto ${puesto_id} para día ${dia}`);
              guardados++;
              continue;
            }
          }
      ...
      logger.error('Error en turno específico::', { turno, error: dbError });
  ...
  logger.debug(`✅ Guardados ${guardados} turnos, eliminados ${eliminados} turnos de ${turnos.length} totales`);
