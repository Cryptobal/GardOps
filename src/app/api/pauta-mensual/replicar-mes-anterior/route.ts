...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🚀 Iniciando replicación de pautas del mes anterior`);
    console.log(`📅 Mes destino: ${mes}/${anio}`);
    ...
    console.log(`📅 Mes origen: ${mesAnterior}/${anioAnterior}`);
    ...
    console.log(`🏢 Instalaciones encontradas: ${instalacionesConPautaResult.rows.length}`);
    ...
    for (const instalacion of instalacionesConPautaResult.rows) {
      ...
      if (puestosResult.rows.length === 0) {
        console.log(`   ⚠️ No hay puestos activos para esta instalación`);
        continue;
      }
      ...
      for (const puesto of puestosResult.rows) {
        console.log(`   🔍 Procesando puesto: ${puesto.nombre_puesto}`);
        ...
        if (pautaMesAnterior.rows.length === 0) {
          console.log(`   ⚠️ No hay pauta del mes anterior para ${puesto.nombre_puesto}, saltando...`);
          continue;
        }

        console.log(`   📊 Pauta del mes anterior: ${pautaMesAnterior.rows.length} días`);
        ...
        console.log(`   🔄 Estados generados para el mes actual: ${estadosMesActual.length} días`);
        ...
        for (let dia = 1; dia <= diasMesActual; dia++) {
          ...
          if (pautaExistente.rows.length > 0) {
            console.log(`   ⚠️ Ya existe pauta para el día ${dia}, actualizando...`);
          ...
          console.log(`   ✅ Día ${dia}: ${estadoNuevo}`);
        }
      ...
      console.log(`   ✅ Instalación ${instalacion.instalacion_nombre}: ${instalacionReplicados} días replicados`);
    }

    console.log('\n🎯 REPLICACIÓN COMPLETADA:');
    console.log('==========================');
    console.log(`✅ Total registros replicados: ${totalReplicados}`);
    console.log(`✅ Instalaciones procesadas: ${resultados.length}`);
...
function calcularContinuidadSerie(patronReal: any, pautaMesAnterior: any[], diasMesActual: number): string[] {
  ...
  for (let dia = 1; dia <= diasMesActual; dia++) {
    ...
    console.log(`   🔄 Día ${dia}: posición en ciclo ${diaEnCiclo}, es trabajo: ${esDiaTrabajo} -> ${esDiaTrabajo ? 'planificado' : 'libre'}`);
  }
