...
export async function GET(request: NextRequest) {
  try {
    logger.debug('🔍 Debug simple - Iniciando...');

    // Test 1: Verificar conexión básica
    const test1 = await sql`SELECT 1 as test`;
    devLogger.search(' Test 1 (conexión):', test1.rows[0]);
    ...
    `;
    devLogger.search(' Test 2 (count):', test2.rows[0]);
    ...
    `;
    devLogger.search(' Test 3 (guardia):', test3.rows[0]);
...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🔍 Debug simple POST - Iniciando...');
    ...
    `;
    devLogger.search(' Guardia encontrado:', guardiaCheck.rows[0]);
    ...
    `;
    devLogger.search(' Todas las estructuras existentes:', existingStructures.rows);

    // Inactivar estructuras activas existentes
    if (existingStructures.rows.some(s => s.activo)) {
      logger.debug('🔍 Inactivando estructuras activas existentes...');
      await sql`
    ...
    `;
    devLogger.search(' Versiones existentes:', existingVersions.rows);

    const nextVersion = existingVersions.rows.length > 0 ? Math.max(...existingVersions.rows.map(r => r.version)) + 1 : 1;
    devLogger.search(' Próxima versión:', nextVersion);
    ...
    devLogger.search(' Inserción exitosa:', result.rows[0]);
    ...
    logger.error('Error en debug simple POST::', error);
    return NextResponse.json(
      { success: false, error: 'Error interno del servidor' },
      { status: 500 }
    );
