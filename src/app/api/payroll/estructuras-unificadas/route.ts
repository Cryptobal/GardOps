...
export async function GET(request: NextRequest) {
  try {
    logger.debug('🔍 GET /api/payroll/estructuras-unificadas - Iniciando...');
    ...
    logger.debug('📊 Parámetros de consulta:', { instalacionId, rolId, guardiaId, tipo, estado });
    ...
    logger.debug('📊 Ejecutando consulta unificada...');
    ...
    logger.debug('Valores:', queryParams);

    const result = await db.query(query, queryParams);
    
    logger.debug('📊 Resultados obtenidos:', result.rows.length);
    logger.debug('📊 Datos de las filas:', result.rows);
...
export async function POST(request: NextRequest) {
  try {
    logger.debug('🔍 POST /api/payroll/estructuras-unificadas - Iniciando...');
    ...
    logger.debug('📝 Datos recibidos:', body);
    ...
    if (tipo === 'servicio') {
      ...
      logger.debug('✅ Estructura de servicio creada exitosamente');
      return NextResponse.json({
        success: true,
      ...
      logger.debug('✅ Estructura de guardia creada exitosamente');
      return NextResponse.json({
        success: true,
