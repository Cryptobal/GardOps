...
export async function POST(request: NextRequest) {
  logger.debug('🔍 POST /api/payroll/estructuras-unificadas/setup - Iniciando...');
  ...
  try {
    logger.debug('📊 Configurando tablas de estructuras unificadas...');

    // Crear tabla sueldo_estructuras_servicio si no existe
    logger.debug('1. Creando tabla sueldo_estructuras_servicio...');
    await query(`
    ...
    logger.debug('2. Creando índices para sueldo_estructuras_servicio...');
    ...
    logger.debug('3. Creando constraint único...');
    try {
      await query(`
      ...
      logger.warn(' Constraint ya existe o error al crearlo:', error);
    }

    // Verificar que la tabla sueldo_estructura_guardia existe
    logger.debug('4. Verificando tabla sueldo_estructura_guardia...');
    ...
    if (!checkGuardia.rows[0].exists) {
      logger.debug('5. Creando tabla sueldo_estructura_guardia...');
      await query(`
    ...
    logger.debug('6. Verificando tabla sueldo_estructura_guardia_item...');
    ...
    if (!checkGuardiaItem.rows[0].exists) {
      logger.debug('7. Creando tabla sueldo_estructura_guardia_item...');
      await query(`
    ...
    logger.debug('8. Verificando estado final...');
    ...
    logger.error('Error configurando tablas de estructuras unificadas::', error);
