...
export async function POST(
  ...
  try {
    ...
    const estructuraResult = await sql`
      ...
      WHERE id = ${estructura_id} AND bono_id IS NULL
      LIMIT 1
    `;
    ...
    const existingItemResult = await sql`
      SELECT id FROM sueldo_estructuras_servicio 
      WHERE estructura_id = ${estructura_id} AND bono_id = ${item_id} AND activo = true
      LIMIT 1
    `;
    ...
    console.error('Error agregando item a estructura:', error);
