...
export async function GET(request: NextRequest) {
  console.log('🔍 GET /api/payroll/instalaciones-con-roles - Iniciando...');
  ...
  try {
    ...
    console.log('📊 Ejecutando consulta de instalaciones...');
    
    const result = await query(instalacionesQuery);

    console.log('📊 Instalaciones encontradas:', result.rows?.length || 0);
