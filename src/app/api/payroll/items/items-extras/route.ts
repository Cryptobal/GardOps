...
try {
    ...
    if (!payrollRunId && (!mes || !anio)) {
      return NextResponse.json(
        { error: 'Se requiere payroll_run_id o la combinación de mes y anio' },
        { status: 400 }
      );
    }
    ...
    if (payrollRunId) {
      ...
      sql += ` AND pr.mes = $${paramIndex} AND pr.anio = $${paramIndex + 1}`;
    ...
    logger.error('Error al obtener ítems extras::', error);
    return NextResponse.json(
...
try {
    ...
    if (!payrollRunId) {
      ...
      const payrollRunResult = await query(`
        ...
        WHERE mes = $1 AND anio = $2 AND tipo = 'PROYECTADA'
      `, [validatedData.mes, validatedData.anio]);

      if (payrollRunResult.rows.length === 0) {
        // Crear nuevo payroll run
        const newPayrollRun = await query(`
          INSERT INTO payroll_run (mes, anio, tipo, fecha_corte, creado_por, cerrado)
          VALUES ($1, $2, 'PROYECTADA', $3, 'system', false)
          RETURNING id
        `, [validatedData.mes, validatedData.anio, new Date()]);
    ...
    logger.error('Error al crear ítem extra::', error);
...
try {
    ...
    logger.error('Error al actualizar ítem extra::', error);
...
export async function DELETE(request: NextRequest) {
  const deny = await requireAuthz(request, { resource: 'payroll', action: 'delete' });
  if (deny) return deny;
...
try {
    ...
    logger.error('Error al eliminar ítem extra::', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
