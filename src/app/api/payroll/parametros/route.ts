...
async function obtenerValorUFActual(): Promise<{ valor: number; fecha: string }> {
  try {
    ...
    logger.error('Error al obtener valor UF desde API::', error);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      logger.warn('parametros: tabla ausente o error, usando vacío');
    }
    ...
    try {
      ...
      logger.warn('afp: tabla/columnas ausentes, usando vacío');
    }
    ...
    try {
      ...
      logger.warn('isapre: tabla/columnas ausentes, usando vacío');
    }
    ...
    try {
      ...
      logger.warn('tramos: tabla ausente, usando vacío');
    }
    ...
    try {
      ...
      logger.warn('uf: error obteniendo desde API, usando default');
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error('Error al actualizar parámetros::', error);
    return NextResponse.json(
      { error: 'Error interno del servidor' },
      { status: 500 }
    );
