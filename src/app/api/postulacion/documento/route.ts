...
async function uploadToR2WithFetch(buffer: Buffer, key: string, contentType: string, metadata: any) {
  ...
  for (let i = 0; i < fetchOptions.length; i++) {
    try {
      logger.debug(`🔄 Intento ${i + 1} de subida a R2 con configuración ${i + 1}`);
      
      const response = await fetch(url, fetchOptions[i]);
      
      if (response.ok) {
        logger.debug(`✅ Archivo subido exitosamente con configuración ${i + 1}`);
        return response;
      } else {
      ...
      lastError = error;
      logger.debug(`❌ Intento ${i + 1} falló:`, error.message);
      
      if (i < fetchOptions.length - 1) {
        logger.debug(`⏳ Esperando 3 segundos antes del siguiente intento...`);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.debug('📤 API Postulación - Subiendo documento:', {
    ...
    try {
      ...
      logger.debug('📤 Subiendo archivo a Cloudflare R2:', key);
      ...
      devLogger.success(' Documento guardado exitosamente:', documentoGuardado);
