...
export async function GET(
  ...
  try {
    ...
    logger.error('Error al obtener rol de servicio::', error);
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    logger.error('Error al actualizar rol de servicio::', error);
    return NextResponse.json(
      { success: false, error: 'Error interno del servidor' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    logger.error('Error al eliminar rol de servicio::', error);
    return NextResponse.json(
      { success: false, error: 'Error interno del servidor' },
      { status: 500 }
    );
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'reactivar') {
      ...
      logger.debug(`✅ Rol de servicio reactivado: ${id}`);
    ...
    logger.error('Error en operación PATCH::', error);
    return NextResponse.json(
