...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tenantId) {
      ...
      logger.debug('⚠️ GET roles-servicio - Sin tenantId, devolviendo todos los roles');
    ...
    devLogger.search(' Query ejecutada:', query);
    devLogger.search(' Parámetros:', params);

    const result = await sql.query(query, params);
    
    devLogger.search(' Resultados obtenidos:', result.rows.length, 'filas');
    ...
    logger.error('Error al obtener roles de servicio::', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!finalTenantId) {
      ...
      devLogger.search(' POST roles-servicio - Email del header:', email);
      if (email) {
        const t = await sql`SELECT tenant_id::text AS tid FROM usuarios WHERE lower(email)=lower(${email}) LIMIT 1`;
        devLogger.search(' POST roles-servicio - Resultado query usuario:', t.rows);
    ...
    devLogger.search(' POST roles-servicio - finalTenantId:', finalTenantId);

    // LÓGICA SIMPLIFICADA - Si es tipo 'series', siempre es horarios variables
    let esRolSeries = (tipo === 'series' || tiene_horarios_variables);
    
    devLogger.search(' Tipo de rol detectado:', { tipo, esRolSeries, tiene_horarios_variables });
    ...
    devLogger.search(' Datos recibidos:', { tipo, duracion_ciclo, dias_serie: dias_serie.length });
    
    // Si es del wizard simple, convertir formato
    if (tipo === 'series' && dias_serie.length > 0) {
      logger.debug('🔄 Procesando datos del wizard simple...');
      ...
      devLogger.success(' Series convertidas:', series_dias.length);
    }
    
    // ELIMINADA VALIDACIÓN PROBLEMÁTICA - Solo validar que tenga series
    devLogger.search(' Series finales:', series_dias.length);
    ...
    devLogger.search(' POST roles-servicio - Datos procesados:', {
      nombreCalculado,
    ...
    if (checkDuplicate.rows.length > 0) {
      logger.warn(' Rol duplicado encontrado:', checkDuplicate.rows[0]);
      return NextResponse.json(
    ...
    try {
      ...
      `);
      logger.debug('✅ Tabla sueldo_estructuras_roles creada/verificada');
    } catch (tableError) {
      logger.warn(' Error creando tabla sueldo_estructuras_roles:', tableError);
    ...
    logger.error('Error al crear rol de servicio::', error);
    return NextResponse.json(
