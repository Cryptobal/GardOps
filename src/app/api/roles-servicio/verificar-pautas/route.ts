...
export async function POST(request: NextRequest) {
  try {
    ...
    devLogger.search(' Endpoint verificar-pautas recibió:', { rol_id, body });

    if (!rol_id) {
      logger.debug('❌ Error: rol_id no proporcionado');
    ...
    if (!isValidUUID(rol_id)) {
      logger.debug('❌ Error: rol_id no es un UUID válido:', rol_id);
      return NextResponse.json(
    ...
    logger.debug('✅ UUID válido, procediendo con la consulta...');
    ...
    devLogger.search(' Resultado de consulta de rol:', rolResult.rows.length, 'filas encontradas');
    
    if (rolResult.rows.length === 0) {
      logger.debug('❌ Rol no encontrado en la base de datos');
      return NextResponse.json({
        tiene_pautas: false,
    ...
    devLogger.success(' Rol encontrado:', rol);
    ...
    devLogger.search(' Pautas encontradas:', pautasCount, 'Tiene pautas:', tienePautas);
    ...
    if (tienePautas) {
      ...
      devLogger.search(' Información detallada de pautas:', pautasInfo);
    }
    ...
    devLogger.success(' Respuesta final:', response);
