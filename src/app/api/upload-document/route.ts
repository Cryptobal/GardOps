...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.debug('📤 Subiendo documento:', {
      archivo: file.name,
      nombre_documento,
      modulo,
      entidad_id,
    ...
    if (isProduction && hasR2Config && s3) {
      try {
        logger.debug('☁️ Subiendo archivo a Cloudflare R2...');
        ...
        devLogger.success(' Archivo subido exitosamente a R2:', r2Url);
      } catch (r2Error: any) {
        ...
        logger.debug('🔄 Fallback: Guardando en base de datos');
      }
    } else {
      if (!isProduction) {
        logger.debug('🔧 Modo desarrollo: Guardando en base de datos');
      } else if (!hasR2Config) {
        logger.debug('⚠️ Configuración R2 incompleta: Guardando en base de datos');
      }
    }
