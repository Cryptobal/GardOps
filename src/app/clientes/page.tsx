...
export default function ClientesPage() {
  ...
  const handleInactivateCliente = async (cliente: Cliente, event?: React.MouseEvent) => {
    ...
    try {
      ...
      if (!checkData.success) {
        logger.error('Error verificando cliente::', checkData.error);
        return;
      }
      ...
      openModal({
        ...
        onConfirm: async (cascada: boolean) => {
          ...
          devLogger.success(' Cliente inactivado:', data.message);
  ...
  const handleActivateCliente = async (cliente: Cliente, event?: React.MouseEvent) => {
    ...
    try {
      ...
      devLogger.success(' Cliente activado:', data.message);
  ...
  const exportarExcel = async () => {
    try {
      ...
      logger.debug('✅ Clientes exportados exitosamente');
    } catch (error) {
  ...
  const descargarPlantilla = async () => {
    try {
      ...
      logger.debug('✅ Plantilla de clientes descargada exitosamente');
    } catch (error) {
  ...
  useEffect(() => {
    const fetchClientes = async () => {
      ...
      try {
        setLoading(true);
        logger.debug('🔍 ClientesPage: Iniciando carga de clientes...');
        devLogger.search(' ClientesPage: Usuario autenticado:', user?.email);
        
        const result = await api.clientes.getAll();

        if (result.success) {
          setClientes(result.data);
          devLogger.success(' ClientesPage: Clientes cargados exitosamente:', result.data.length);
        } else {
...
logger.debug("✅ Vista principal de clientes rediseñada correctamente"); 