...
export default function RolDetallePage() {
  ...
  useEffect(() => {
    ...
    logger.debug('Cambios pendientes detectados:', cambiosPendientes, 'Hay cambios:', hayCambios);
  ...
  const guardarCambios = async () => {
    ...
    try {
      ...
      logger.debug('🔧 Aplicando cambios pendientes:', cambiosPendientes);
      logger.debug('📊 Permisos originales:', permisosOriginales.size);
      ...
      Object.entries(cambiosPendientes).forEach(([modulo, nivel]) => {
        ...
        logger.debug(`🗑️ Removidos ${removidos} permisos de ${modulo}`);

        // Agregar nuevos permisos para el nivel
        const permisosIds = obtenerPermisosParaNivel(modulo, nivel);
        logger.debug(`➕ Agregando ${permisosIds.length} permisos para ${modulo} nivel ${nivel}`);
        permisosIds.forEach(id => nuevosPermisos.add(id));
      });

      const permisosArray = Array.from(nuevosPermisos);
      logger.debug('Enviando permisos al servidor:', permisosArray.length, 'permisos');
      logger.debug('Cambios pendientes aplicados:', cambiosPendientes);
      ...
      logger.debug('🔄 FORZANDO RECARGA COMPLETA DE LA PÁGINA...');
