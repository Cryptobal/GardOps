...
export default function RolesPage() {
  ...
  const guardarRol = async () => {
    ...
    try {
      ...
      toast ? toast({ title: 'Rol creado exitosamente', type: 'success' }) : logger.warn('Rol creado exitosamente');
      cancelarEdicion();
      await cargarRoles();
    } catch (error) {
      console.error('Error:', error);
      toast ? toast({ title: 'Error', description: String((error as any)?.message ?? 'Error al guardar rol'), type: 'error' }) : logger.warn('Error al guardar rol');
    } finally {
      setGuardando(false);
    }
  };
