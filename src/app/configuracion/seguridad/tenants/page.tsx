...
export default function TenantsPage() {
  ...
  async function createTenant() {
    try {
      ...
      toast ? toast({ title: 'Éxito', description: successMsg, type: 'success' }) : logger.warn(successMsg);
      setForm({ nombre: '', rut: '', admin_email: '', admin_nombre: '', admin_password: '' });
      await loadTenants();
    } catch (e: any) {
      const msg = 'Error creando tenant';
      toast ? toast({ title: 'Error', description: msg, type: 'error' }) : logger.warn(msg, e?.message);
    } finally {
      setSubmitting(false);
    }
  }
