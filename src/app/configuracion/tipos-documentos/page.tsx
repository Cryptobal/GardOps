...
export default function TiposDocumentosPage() {
  ...
  const cargarTipos = async () => {
    try {
      setCargando(true);
      logger.debug('🔄 Cargando tipos de documentos...');
      ...
      if (data.success) {
        setTipos(data.data);
        logger.debug(`✅ ${data.data.length} tipos cargados exitosamente`);
      } else {
  ...
  const abrirModal = (tipo?: TipoDocumento) => {
    if (tipo) {
      ...
      logger.debug('✏️ Editando tipo:', tipo);
    } else {
      setEditando(null);
      setFormData({ 
        modulo: "", 
      ...
      logger.debug('➕ Creando nuevo tipo');
    }
  ...
  const abrirModalEliminar = (tipo: TipoDocumento) => {
    ...
    logger.debug('🗑️ Preparando eliminación de:', tipo);
  };
  ...
  const guardarTipo = async () => {
    ...
    try {
      ...
      logger.debug(`${editando ? '🔄' : '➕'} ${method} ${url}`, formData);
  ...
  const eliminarTipo = async () => {
    ...
    try {
      ...
      logger.debug('🗑️ Eliminando tipo:', tipoAEliminar.id);
      ...
      const data = await response.json();
      logger.debug('📄 Respuesta eliminación:', data);
