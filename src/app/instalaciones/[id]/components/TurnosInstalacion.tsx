...
export default function TurnosInstalacion({ 
    ...
    try {
      ...
      const promises = [
        getTurnosInstalacion(instalacionId).catch(error => {
          logger.error('Error cargando turnos::', error);
          return [];
        }),
        getRolesServicio().catch(error => {
          ...
          logger.debug('Roles cargados:', roles);
          return Array.isArray(roles) ? roles : [];
        }),
        getPPCsInstalacion(instalacionId).catch(error => {
          logger.error('Error cargando PPCs::', error);
          return [];
        }),
        getGuardiasDisponibles(
  ...
  const handleEliminarPuesto = (ppcId: string, puestoIndex: number) => {
    devLogger.search(' handleEliminarPuesto llamado:', { ppcId, puestoIndex });
    setShowDeleteConfirm({ ppcId, puestoIndex });
  };

  const confirmarEliminarPuesto = async () => {
    devLogger.search(' confirmarEliminarPuesto llamado:', showDeleteConfirm);
    ...
    try {
      ...
      devLogger.search(' Resultado de eliminarPPC:', resultado);
  ...
  const handleEditarNombrePuesto = async (ppcId: string, nombreActual: string) => {
    ...
    try {
      ...
      logger.error('Error cargando tipos de puesto::', error);
    }
  };
  ...
  const handleGuardarNombrePuesto = async () => {
    ...
    try {
      ...
      logger.error('Error actualizando nombre del puesto::', error);
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDeleteModal
        ...
        onClose={() => {
          logger.debug('🔍 Cerrando modal de eliminar puesto');
      ...
      {(() => { logger.debug("✅ Refactor visual de asignación de turnos completado sin afectar la lógica"); return null; })()}
