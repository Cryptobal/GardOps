...
export default function InstalacionesPage() {
  ...
  const exportarExcel = async () => {
    try {
      ...
      logger.debug('✅ Instalaciones exportadas exitosamente');
    } catch (error) {
      logger.error('Error exportando instalaciones::', error);
    }
  };
  ...
  const descargarPlantilla = async () => {
    try {
      ...
      logger.debug('✅ Plantilla de instalaciones descargada exitosamente');
    } catch (error) {
  ...
  const fetchInstalaciones = useCallback(async () => {
    try {
      ...
      if (result.success) {
        devLogger.search(' Datos de instalaciones recibidos:', result.data);
  ...
  const fetchKPIs = useCallback(async () => {
    try {
      logger.debug('🔍 Cargando KPIs de instalaciones...');
      const response = await fetch('/api/instalaciones/kpis');
      const data = await response.json();
      
      if (data.success) {
        devLogger.success(' KPIs cargados exitosamente:', data.data);
        setKpis(data.data);
      } else {
  ...
  useEffect(() => {
    logger.debug('🎯 KPIs actuales:', kpis);
  }, [kpis]);
  ...
  const handleInactivateInstalacion = useCallback(async (instalacion: any, event?: React.MouseEvent) => {
    devLogger.search(' handleInactivateInstalacion llamado para:', instalacion.nombre);
    if (event) {
    ...
    try {
      ...
      if (!checkData.success) {
        logger.error('Error verificando instalación::', checkData.error);
        return;
      }

      const { can_inactivate, blockers = [] } = checkData.data;

      devLogger.search(' Llamando a inactivateInstalacion con:', instalacion.nombre);
      await inactivateInstalacion(
        instalacion.nombre,
        async () => {
          logger.debug('🔍 Función de confirmación ejecutada');
          ...
          devLogger.success(' Instalación inactivada:', data.message);
      ...
      logger.error('Error en inactivación::', error);
    }
  ...
  const handleActivateInstalacion = useCallback(async (instalacion: any, event?: React.MouseEvent) => {
    ...
    try {
      ...
      devLogger.success(' Instalación activada:', data.message);
