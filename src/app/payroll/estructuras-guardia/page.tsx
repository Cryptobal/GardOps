...
export default function EstructurasGuardiaPage() {
  ...
  const cargarEstructurasInactivas = async () => {
    ...
    try {
      ...
      logger.error('Error cargando estructuras inactivas::', e);
    } finally {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoadingInactiveList(true);
      devLogger.search(' Cargando estructuras inactivas...', { selectedGuardia, showInactiveStructures });
      try {
        const res = await fetch(`/api/payroll/estructuras-guardia/list?guardia_id=${selectedGuardia}`);
        const json = await res.json();
        devLogger.search(' Respuesta de estructuras:', json);
        if (json.success) {
          const estructuras = json.data.estructuras || [];
          devLogger.search(' Estructuras cargadas:', estructuras);
          setAllStructures(estructuras);
      }
    } catch (error) {
        logger.error('Error cargando estructuras inactivas::', error);
      } finally {
        setLoadingInactiveList(false);
      }
    };
  ...
  const cargarItemsGlobales = async () => {
    try {
      ...
      logger.error('Error cargando ítems globales::', error);
  ...
  const cargarEstructura = async () => {
    ...
    try {
      ...
      logger.error('Error cargando estructura::', error);
  ...
  const handleEliminarEstructura = async (id: string) => {
    ...
    try {
      ...
      logger.error('Error eliminando estructura::', error);
