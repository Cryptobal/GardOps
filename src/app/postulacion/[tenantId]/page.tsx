...
export default function PostulacionPage() {
  ...
  const verificarRutDuplicado = async (rut: string): Promise<boolean> => {
    try {
      ...
      if (response.ok) {
        ...
        logger.error('Error en respuesta de verificación RUT::', response.status, response.statusText);
        return false;
      }
    } catch (error) {
      logger.error('Error verificando RUT::', error);
      return false;
    }
  };
  ...
  const handleInputChange = (field: keyof FormData, value: string | number) => {
    logger.debug(`Campo ${field} cambiado a:`, value);
  ...
  const enviarFormulario = async () => {
    ...
    logger.debug('🚀 Enviando formulario sin validación previa de RUT');
    ...
    setLoading(true);
    try {
      logger.debug('🚀 Enviando formulario a:', '/api/postulacion/crear');
      logger.debug('📋 Datos del formulario:', { ...formData, tenant_id: tenantId });
      ...
      if (!guardiaResponse.ok) {
        ...
        logger.debug('❌ Error del servidor:', errorData);
