...
export default function PPCModal({ 
  ...
  const cargarPPCs = async () => {
    try {
      ...
      console.log(`✅ Pauta del día cargada:`, {
      ...
      console.error('Error cargando pauta del día:', error);
  ...
  const handleAsignarPermanente = async (ppc: PPC) => {
    try {
      setAsignando(ppc.id);
      console.log('🟦 Asignación permanente:', { guardia_id: guardia.id, puesto_operativo_id: ppc.id });
      ...
      console.error('Error asignando guardia permanente:', error);
  ...
  const handleTurnoExtraPPC = async (ppc: PPC) => {
    try {
      ...
      console.error('Error creando turno extra PPC:', error);
  ...
  const handleTurnoExtraReemplazo = async (turno: TurnoAsignado) => {
    try {
      ...
      console.error('Error creando turno extra reemplazo:', error);
