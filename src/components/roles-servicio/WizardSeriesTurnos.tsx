...
export default function WizardSeriesTurnos({
  ...
  // Debug
  devLogger.search(' WIZARD SERIES - Estado:', { paso, duracion, dias: dias.length });
  ...
  const handleCrearRol = async () => {
    try {
      setLoading(true);
      logger.debug('🚀 CREANDO ROL SERIE...');
      ...
      logger.debug('🚀 Enviando datos:', rolData);
      
      logger.debug('🚀 Llamando onSave con:', rolData);
  ...
  const aplicarATodos = () => {
    logger.debug('🔧 APLICANDO A TODOS...');
    
    const inicio = inicioTodosRef.current?.value || '08:00';
    const fin = finTodosRef.current?.value || '20:00';
    
    logger.debug('📅 Valores:', { inicio, fin });
    logger.debug('📅 setDias function:', typeof setDias);
    
    // Actualizar días de trabajo
    setDias(prev => {
      logger.debug('📅 Días anteriores:', prev);
      const nuevosDias = prev.map(d => 
        d.trabaja ? { ...d, inicio, fin } : d
      );
      logger.debug('📅 Días nuevos:', nuevosDias);
      return nuevosDias;
    });
    ...
    logger.debug('✅ APLICADO EXITOSAMENTE');
  };
