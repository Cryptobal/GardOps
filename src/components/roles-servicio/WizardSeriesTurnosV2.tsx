...
export default function WizardSeriesTurnosV2({ isOpen, onClose, onSave }: WizardProps) {
  ...
  React.useEffect(() => {
    if (isOpen) {
      devLogger.process(' Modal abierto - inicializando días para duración:', duracion);
      ...
      setDias(nuevosDias);
      devLogger.search(' Días inicializados:', nuevosDias.length);
    }
  }, [duracion, isOpen]);
  ...
  React.useEffect(() => {
    if (isOpen) {
      logger.debug('🔄 Modal abierto - reset completo');
      setPaso(1);
      setLoading(false);
  ...
  const handleCrearRol = useCallback(async () => {
    try {
      ...
      logger.debug('🔄 Reset forzado después de crear rol');
      setPaso(1);
      setDuracion(8);
      setDias([]);
      setHoraInicioTodos('08:00');
  ...
  const handleCerrar = useCallback(() => {
    logger.debug('🔄 Cerrando wizard - reseteando estados');
    setPaso(1);
    setDuracion(8);
    setDias([]);
    setHoraInicioTodos('08:00');
