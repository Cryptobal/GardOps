...
export function DocumentManager({ 
  ...
  const cargarDocumentos = useCallback(async (forceReload: boolean = false) => {
    ...
    try {
      devLogger.search(' CARGANDO DOCUMENTOS:', { modulo, entidadId, forceReload });
      setCargando(true);
      ...
      logger.debug('🌐 Llamando API:', apiUrl);
      
      const response = await fetch(apiUrl, {
        cache: 'no-store'
      });
      
      logger.debug('📡 Respuesta API:', { status: response.status, ok: response.ok });
      ...
      if (data.success) {
        ...
        logger.debug('📄 Documentos mapeados:', docsConTipo);
  ...
  const cargarTiposDocumentos = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        logger.error('Error cargando tipos::', data.error);
        setTiposDocumentos([]);
      }
    } catch (error) {
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        devLogger.success(' Documento subido exitosamente:', result.data);
  ...
  useEffect(() => {
    devLogger.process(' useEffect ejecutado:', { entidadId, refreshTrigger, modulo });
    
    // Solo cargar documentos si hay una entidad seleccionada
    if (entidadId && entidadId.trim() !== "") {
      logger.debug('✅ Entidad válida, cargando documentos...');
      cargarDocumentos(true);
      cargarTiposDocumentos();
    } else {
      logger.debug('❌ Entidad inválida o vacía, limpiando documentos');
