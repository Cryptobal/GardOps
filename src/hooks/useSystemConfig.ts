...
export function useSystemConfig() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          logger.warn('No se pudo cargar configuración del sistema, usando valores por defecto');
          setConfig(defaultConfig);
        }
      } else {
        logger.warn('Error cargando configuración del sistema, usando valores por defecto');
        setConfig(defaultConfig);
      }
      setError(null);
    } catch (err) {
      logger.error('Error cargando configuración del sistema::', err);
  ...
  const formatTimeRange = (startTime: string, endTime: string): string => {
    try {
      ...
      logger.error('Error formateando rango de tiempo::', error);
