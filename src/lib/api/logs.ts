...
export const getLogs = async (modulo: string, entidadId: string): Promise<Log[]> => {
  try {
    logger.debug(`🔄 Obteniendo logs para módulo: ${modulo}, entidad: ${entidadId}`);
    ...
    if (data.success) {
      logger.debug(`✅ ${data.data.length} logs obtenidos para ${modulo}`);
      return data.data;
    } else {
...
export const logAccion = async (logData: LogRequest): Promise<boolean> => {
  try {
    logger.debug(`📝 Registrando log para módulo: ${logData.modulo}`, logData);
    ...
    if (data.success) {
      logger.debug('✅ Log registrado exitosamente');
      return true;
    } else {
