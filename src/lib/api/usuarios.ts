...
export async function authenticateUser(credentials: LoginCredentials): Promise<AuthResponse | null> {
  try {
    ...
    try {
      ...
      logger.error('Error en autenticación Vercel Postgres::', e);
    }
    ...
    if (!user) {
      try {
        ...
        logger.error('Error en autenticación fallback::', e);
      }
    }
...
export async function initializeDefaultUsers(): Promise<void> {
  logger.debug('🚫 initializeDefaultUsers DESHABILITADA - Los usuarios se crean manualmente desde el frontend');
  logger.debug('✅ No se crearán usuarios automáticamente');
  return;
} 