...
export function getCurrentUserServer(req: NextRequest): CurrentUserServer | null {
  try {
    ...
    if (!token) {
      logger.debug('🔍 getCurrentUserServer: No se encontró token');
      return null;
    }
    ...
    if (!payload) {
      logger.debug('🔍 getCurrentUserServer: Token inválido');
      return null;
    }
...
export async function requirePermission(permission: string): Promise<void> {
  ...
  if (process.env.NODE_ENV === 'development') {
    devLogger.search(' [requirePermission] Modo desarrollo - permitiendo permiso:', permission);
    return;
  }
  
  // BYPASS TEMPORAL PARA PRODUCCIÓN - TURNOS
  if (process.env.NODE_ENV === 'production' && permission === 'turnos.marcar_asistencia') {
    logger.debug('🔍 [requirePermission] Bypass temporal para turnos.marcar_asistencia en producción');
    return;
  }
  ...
  try {
    ...
    if (!email) {
      logger.debug('❌ [requirePermission] No se pudo obtener email del usuario');
    ...
    if (!userId) {
      logger.debug('❌ [requirePermission] Usuario no encontrado:', email);
    ...
    if (!hasPermission) {
      ...
      if (!isPlatformAdmin) {
        logger.debug('❌ [requirePermission] Permiso denegado:', permission, 'para usuario:', email);
        throw new Error('FORBIDDEN');
      }
    }
    
    devLogger.success(' [requirePermission] Permiso concedido:', permission, 'para usuario:', email);
  } catch (error: any) {
