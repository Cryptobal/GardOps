...
export async function requireAuthz(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development' && userEmail) {
      devLogger.search(' Auth: Usando email de desarrollo:', userEmail);
      ...
      if (userResult.rows.length === 0) {
        logger.debug('❌ Auth: Usuario no encontrado por email:', userEmail);
      ...
      devLogger.success(' Auth: Usuario encontrado por email:', user.email);
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        if (isPublicRoute) {
          logger.debug('🔍 Auth: Ruta pública en producción, permitiendo acceso temporal');
    ...
    if (!user) {
      logger.debug('🔍 Auth: Sin usuario, permitiendo acceso a ruta pública');
      return null;
    }
    ...
    devLogger.search(' Auth: Permisos del usuario:', userPermissions);
    
    // Verificar si el usuario tiene el permiso requerido
    const requiredPerm = toPermCode(permission.resource, permission.action);
    devLogger.search(' Auth: Permiso requerido:', requiredPerm);
    ...
    if (!hasPermission) {
      logger.debug('❌ Auth: Permiso denegado. Requerido:', requiredPerm, 'Disponibles:', userPermissions);
      return NextResponse.json(
        { 
    ...
    devLogger.success(' Auth: Permiso concedido para:', requiredPerm);
