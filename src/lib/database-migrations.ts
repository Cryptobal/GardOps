...
export async function migrateGuardiasWithData(): Promise<MigrationResult> {
  ...
  try {
    console.log('🔄 Iniciando migración preservando datos de guardias...');
    ...
    if (parseInt(tenantCount.rows[0].count) === 0) {
      ...
      console.log('✅ Tenant por defecto "GardOps" creado');
    }

    // 2. Obtener datos actuales de guardias
    const guardiasData = await query('SELECT * FROM guardias');
    console.log(`📊 Encontrados ${guardiasData.rows.length} registros en guardias`);
    ...
    console.log(`✅ ${guardiasData.rows.length} registros migrados a guardias_temp`);
    ...
    console.log('✅ Tabla guardias recreada con UUIDs preservando todos los datos');
...
export async function createPautasDiariasTable(): Promise<MigrationResult> {
  ...
  try {
    ...
    `);
    console.log('✅ Índice idx_pautas_diarias_pauta_mensual creado');
...
export async function createPuestosPorCubrirTable(): Promise<MigrationResult> {
  ...
  try {
    ...
    `);
    console.log('✅ Índice idx_ppc_pauta_diaria creado');
...
export async function createUsuariosTable(): Promise<MigrationResult> {
  ...
  try {
    ...
    if (!exists) {
      ...
      console.log('✅ Tabla usuarios creada con todas las columnas');
    ...
    console.log('🔧 Verificando columnas de tabla usuarios existente...');
    ...
    console.log('✅ Tabla usuarios verificada y actualizada con todas las columnas');
...
export async function runDatabaseMigrations(preserveData: boolean = false): Promise<MigrationResult> {
  ...
  try {
    ...
    console.log('🔍 Validando tablas requeridas...');
