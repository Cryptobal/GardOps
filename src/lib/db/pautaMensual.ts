...
export async function aplicarPermisoEnPautaMensual({
  ...
  logger.debug(`🔧 Aplicando permiso ${tipoPermiso} para guardia ${guardiaId} desde ${fechaInicio} hasta ${fechaFin}`);
  ...
  try {
    ...
    logger.debug(`✅ Permiso ${tipoPermiso} aplicado en ${result.rowCount} días de la pauta`);
