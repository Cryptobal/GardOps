...
export async function registrarPermiso({ guardiaId, tipo, desde, hasta, observaciones }: {
  ...
  logger.debug(`🔧 Registrando permiso ${tipo} para guardia ${guardiaId} desde ${desde} hasta ${hasta}`);
...
export async function registrarFiniquito({ guardiaId, fecha_termino }: {
  ...
  logger.debug(`🔧 Registrando finiquito para guardia ${guardiaId} desde ${fecha_termino}`);
  ...
  try {
    ...
    logger.debug(`✅ Eliminados ${resultPauta.rowCount} registros de pauta mensual`);
    ...
    for (const turno of turnosEliminados.rows) {
      ...
      if (puestosDisponibles.rows.length > 0) {
        ...
        ppcsCreados++;
        logger.debug(`✅ PPC creado para puesto ${puestosDisponibles.rows[0].nombre_puesto} en ${turno.fecha}`);
      } else {
        logger.debug(`⚠️ No se encontró puesto operativo disponible para ${turno.instalacion_id} - ${turno.rol_servicio_id}`);
      }
    }

    logger.debug(`✅ Creados ${ppcsCreados} PPCs por finiquito`);
    return ppcsCreados;
  } catch (error) {
...
export async function obtenerPermisos({ guardiaId, tipo }: {
  ...
  logger.debug(`🔍 Obteniendo permisos para guardia ${guardiaId}${tipo ? ` tipo ${tipo}` : ''}`);
  ...
  try {
    ...
    logger.debug(`✅ Obtenidos ${result.rows.length} permisos`);
