...
export async function cargarGoogleMaps(): Promise<boolean> {
  try {
    ...
    if (typeof google !== 'undefined' && google.maps) {
      logger.debug('Google Maps ya está cargado');
      return true;
    }

    logger.debug('Cargando Google Maps dinámicamente...');

    // Usar el manager centralizado
    const manager = GoogleMapsManager.getInstance();
    await manager.load();
    
    logger.debug('Google Maps cargado dinámicamente exitosamente');
    return true;
  } catch (error) {
...
export async function geocodificarDireccion(direccion: string): Promise<GeocodingResult | null> {
  try {
    ...
    logger.debug('Google Maps está disponible, creando geocoder...');
      ...
      geocoder.geocode({ 
        ...
        if (status === 'OK' && results && results[0]) {
          ...
          if (result.address_components) {
            logger.debug('Componentes de dirección encontrados:', result.address_components);
            ...
            result.address_components.forEach((component) => {
              ...
              logger.debug('Componente:', name, 'Tipos:', types);
            ...
            if (administrativeLevel1) {
              ...
              logger.debug('Región encontrada:', administrativeLevel1);
            }
            ...
            if (!componentes.comuna) {
              ...
              for (const part of direccionParts) {
                ...
                if (part.includes('Lo Barnechea') || 
                  ...
                  logger.debug('Comuna extraída de dirección formateada:', part);
                  break;
                }
              }
            }
            ...
            if (!componentes.ciudad && componentes.region) {
              ...
              logger.debug('Usando región como ciudad:', componentes.region);
            }
          }
          ...
          logger.debug('Geocodificación exitosa:', geocodingResult);
          resolve(geocodingResult);
        } else if (status === 'ZERO_RESULTS') {
          logger.warn('No se encontraron resultados para la dirección:', direccion);
          ...
          logger.debug('Intentando variaciones:', variaciones);
          ...
          const intentarVariacion = (index: number) => {
            ...
            logger.debug(`Intentando variación ${index + 1}:`, variacion);
            ...
            geocoder.geocode({ 
              ...
              if (status2 === 'OK' && results2 && results2[0]) {
                ...
                logger.debug('Geocodificación parcial exitosa con variación:', variacion, geocodingResult);
                resolve(geocodingResult);
              } else {
                logger.debug(`Variación ${index + 1} falló, intentando siguiente...`);
          ...
          logger.error('Error en geocodificación::', status);
