  ...
  const preloadCommonPermissions = useCallback(async () => {
    ...
    try {
      ...
      */
      ...
      // logger.error('Error precargando permisos::', error);
      if (mounted.current) {
        setLoading(false);
        setInitialized(true);
      }
  ...
  const preloadPermissions = useCallback(async (perms: string[]) => {
    ...
    const permissionPromises = uncachedPerms.map(async (perm) => {
      try {
        ...
        // logger.warn(`Error cargando permiso ${perm}:`, error);
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (mounted.current && loading) {
        // logger.warn('Timeout cargando permisos, inicializando con permisos por defecto');
        setLoading(false);
        setInitialized(true);
      }
