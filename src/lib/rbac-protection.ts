...
export async function cleanOrphanedRelations(): Promise<RBACProtectionResult> {
  ...
  try {
    console.log('🛡️ [RBAC Protection] Iniciando limpieza de relaciones huérfanas...');
    ...
    if (orphanedRoles.rows.length > 0) {
      console.log(`🛡️ [RBAC Protection] Eliminadas ${orphanedRoles.rows.length} relaciones con roles inexistentes`);
    ...
    if (usersWithoutRole.rows.length > 0) {
      ...
      console.log(`🛡️ [RBAC Protection] Advertencia: ${usersWithoutRole.rows.length} usuarios sin rol:`);
    ...
    result.success = true;
    console.log('🛡️ [RBAC Protection] Limpieza completada exitosamente');
...
export async function validateRBACConsistency(): Promise<RBACProtectionResult> {
  ...
  try {
    console.log('🛡️ [RBAC Protection] Validando consistencia del sistema...');
    ...
    console.log(`🛡️ [RBAC Protection] Validación completada. Inconsistencias: ${result.warnings.length}`);
...
export async function autoProtectRBAC(): Promise<void> {
  try {
    ...
    if (!cleanResult.success || !validateResult.success) {
      console.warn('🛡️ [RBAC Protection] Se encontraron inconsistencias que requieren atención manual');
    }
  } catch (error) {
