...
export function calcularHorasSemanales(rol: RolServicio, seriesDias?: any[]): CalculoHorasSemanales {
  ...
  devLogger.search(' Calculando horas para rol:', { 
    nombre: rol.nombre, 
  ...
  if (rol.tiene_horarios_variables && seriesDias && seriesDias.length > 0) {
    ...
    devLogger.search(' Debug duración ciclo:', { 
      duracion_ciclo_dias: duracionCicloRaw,
      dias_trabajo: rol.dias_trabajo,
      dias_descanso: rol.dias_descanso,
      calculada: duracionCicloCalculada,
      final: duracionCiclo
    });
    ...
    devLogger.search(' Cálculo desde series:', { totalHorasCiclo, duracionCiclo, horasSemanales });
  } else {
    ...
    devLogger.search(' Cálculo tradicional:', { horasPorDia, diasTrabajo, diasDescanso, horasSemanales });
  }
  
  // Validar que no sea NaN
  if (isNaN(horasSemanales) || horasSemanales <= 0) {
    logger.warn('⚠️ Horas semanales inválidas, usando fallback');
