...
export async function geocodificarLote(
  ...
  console.log(`🗺️ Iniciando geocodificación de ${direcciones.length} direcciones...`);
  
  for (let i = 0; i < direcciones.length; i++) {
    const direccion = direcciones[i];
    console.log(`📍 Geocodificando ${i + 1}/${direcciones.length}: ${direccion}`);
  ...
  console.log(`✅ Geocodificación completada: ${exitosos}/${direcciones.length} exitosos`);
...
export async function actualizarCoordenadasGuardia(
  ...
  try {
    ...
    console.log(`✅ Coordenadas actualizadas para guardia ${guardiaId}`);
  } catch (error) {
...
export async function obtenerGuardiasSinCoordenadas(): Promise<Array<{
  ...
  try {
    ...
    console.error('Error obteniendo guardias sin coordenadas:', error);
    throw error;
  }
}
